...
bool Player_is_winner(int player)
{
  ...
  // TODO: Check to see any row has all its cells matching the player marker 
  for(r=BOARD_START_ROW; r<BOARD_ROWS; r++){
    if(Board_scan_line_for_marker(r, BOARD_START_COL, r, BOARD_END_COL, BOARD_SCAN_DIR_RIGHT, BOARD_SCAN_DIR_NONE, m) == true){
      return true;
    }
  }
  // TODO: Check to see any col has all its cells matching the player marker 
  for(c=BOARD_START_COL; c < BOARD_COLS; c++){
    if(Board_scan_line_for_marker(BOARD_START_ROW, c, BOARD_END_ROW, c, BOARD_SCAN_DIR_NONE, BOARD_SCAN_DIR_DOWN, m) == true){
      return true;
    }
  }
  // TODO: check last upper left to lower right diagonal
  if(Board_scan_line_for_marker(BOARD_START_ROW, BOARD_START_COL, BOARD_END_ROW, BOARD_END_COL, BOARD_SCAN_DIR_RIGHT, BOARD_SCAN_DIR_DOWN, m) == true){
    return true;
  }
  // TODO: check last upper right to lower left diagonal
  if(Board_scan_line_for_marker(BOARD_START_ROW, BOARD_END_COL, BOARD_END_ROW, BOARD_START_COL, BOARD_SCAN_DIR_LEFT, BOARD_SCAN_DIR_DOWN, m) == true){
    return true;
  }
...
{
  // TODO: Base case: last element in pattern
  ...
  // TODO: process current element and reduce to the remaining elements 
  if(Board[r][c] != m){
    return false;
  }else{
    return Board_scan_line_for_marker(r+vertical_dir, c+horizontal_dir, end_row, end_col, horizontal_dir, vertical_dir, m); 
  }
}
