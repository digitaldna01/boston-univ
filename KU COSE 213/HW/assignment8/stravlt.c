...
static NODE *_insert( NODE *root, NODE *newPtr){
	...
	if(strcmp(newPtr->data, root->data) < 0){
		...
		if(getHeight(root->left) - getHeight(root->right) > BALANCING){
			if(strcmp(newPtr->data, root->left->data) < 0){
		...
		if(getHeight(root->right) - getHeight(root->left) > BALANCING){
			if(strcmp(newPtr->data, root->right->data) > 0){
...
static NODE *_makeNode( char *data){
	...
	char *person = (char *)malloc(strlen(data) + 1);
	strcpy(person, data); 
	newNode->data = person;
    newNode->left = NULL;
    newNode->right = NULL;
