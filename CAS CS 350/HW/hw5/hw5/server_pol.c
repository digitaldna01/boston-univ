...
void queue_init(struct queue * the_queue, size_t queue_size)
{
	...
	the_queue->requests = (struct request_meta *)malloc(sizeof(struct request_meta) * queue_size);
...
int add_to_queue(struct request_meta to_add, struct queue * the_queue, struct connection_params conn_params)
{
	...
	if (the_queue->available == 0) {
		...
		size_t count = the_queue->max_size - the_queue->available;
    	for (size_t i = 0; i < count; i++) {
        	size_t index = (the_queue->rd_pos + i) % the_queue->max_size;
        	if (the_queue->requests[index].request.req_id == to_add.request.req_id) {
            	return 0; // Already in the queue
        	}
    	}

		if (conn_params.policy == QUEUE_SJN) {
    		size_t insert_position = the_queue->wr_pos;
    		size_t count = the_queue->max_size - the_queue->available;
    		for (size_t i = 0; i < count; i++) {
    		    size_t current_index = (the_queue->rd_pos + i) % the_queue->max_size;
    		    if (TSPEC_TO_DOUBLE(to_add.request.req_length) < TSPEC_TO_DOUBLE(the_queue->requests[current_index].request.req_length)) {
    		        insert_position = current_index;
    		        break;
    		    }
    		}

    		if (insert_position != the_queue->wr_pos) {
    		    // Move below
    		    for (size_t i = the_queue->wr_pos; i != insert_position; ) {
    		        size_t prev = (i - 1 + the_queue->max_size) % the_queue->max_size;
    		        the_queue->requests[i] = the_queue->requests[prev];
    		        i = prev;
    		    }
    		}
    		the_queue->requests[insert_position] = to_add;
		} else {
		    // FIFO
		    ...
		    struct worker_params * common_params)
{
	...
	if (cmd == WORKERS_START) {
		...
		for (i = 0; i < worker_count; ++i) {
			// worker_id needed later to check if the id is started later
			worker_ids[i] = -1;
		...
		for (i = 0; i < worker_count; ++i) {
			worker_ids[i] = pthread_create(&worker_pthreads[i], NULL, worker_main, (void*)worker_params[i]);
	...
	while((opt = getopt(argc, argv, "q:w:p:")) != -1) {
		...
		case 'p':
			if (strcmp(optarg, "FIFO") == 0) {
                    conn_params.policy = QUEUE_FIFO;
					printf("INFO: setting queue policy = FIFO\n");
                } else if (strcmp(optarg, "SJN") == 0) {
                    conn_params.policy = QUEUE_SJN;
					printf("INFO: setting queue policy = SJN\n");
                } else {
                    fprintf(stderr, "ERROR: invalid queue policy '%s'\n", optarg);
                    exit(EXIT_FAILURE);
                }
			break;
		default: /* '?' */
