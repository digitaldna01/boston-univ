...
struct connection_params {
	...
	enum hardware_event hardware_event;
};
...
void * worker_main (void * arg)
{
	...
	/* Set up event counting for the calling thread */
    evt_fd = setup_perf_counter(type, config);

    if (evt_fd < 0) {
        ERROR_INFO();
        perror("Unable to setup performance counter");
        return NULL;
    }
    if (ioctl(evt_fd, PERF_EVENT_IOC_ENABLE, 0) < 0) {
        ERROR_INFO();
        perror("Unable to enable performance counter");
        return NULL;
    }
    // printf("[#WORKER#] Performance counter set up successfully (fd: %d)\n", evt_fd);
	...
	while (!params->worker_done) {
		...
		uint64_t img_id, hardware_counter;
		...
		via perflib.
		 */
		if (ioctl(evt_fd, PERF_EVENT_IOC_RESET, 0) < 0) {
            ERROR_INFO();
            perror("Unable to reset performance counter");
            return NULL;
        }
		...
		hardware_counter = read_perf_counter(evt_fd);
		    ...
		    struct worker_params * common_params)
{
	...
	if (cmd == WORKERS_START) {
		...
		for (i = 0; i < worker_count; ++i) {
			...
			worker_params[i]->hardware_event = common_params->hardware_event;
		}
...
void handle_connection(int conn_socket, struct connection_params conn_params)
{
	...
	common_worker_params.hardware_event = conn_params.hardware_event;
	...
	while((opt = getopt(argc, argv, "q:w:p:h:")) != -1) {
		...
		case 'h':
			if (!strcmp(optarg, "INSTR")){
				conn_params.hardware_event = HARDWARE_INSTR;
			} else if (strcmp(optarg, "L1MISS") == 0){
				conn_params.hardware_event = HARDWARE_L1MISS;
			} else if (!strcmp(optarg, "LLCMISS")){
				conn_params.hardware_event = HARDWARE_LLCMISS;
			} else {
				ERROR_INFO();
				fprintf(stderr, "Invalid Hardware event type.\n" USAGE_STRING, argv[0]);
				return EXIT_FAILURE;
			}
			printf("INFO: setting hardware event type = %s\n", optarg);
			break;
		default: /* '?' */
