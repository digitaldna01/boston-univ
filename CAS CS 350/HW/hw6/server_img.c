...
struct queue {
	...
	struct image * image_store[MAX_IMAGES];
...
uint64_t registerImage(struct queue * the_queue, struct image* new_image) {
	...
	uint64_t img_id = the_queue->next_img_id;
    
    // Check if the image store is full
    if (img_id >= MAX_IMAGES) {
        return UINT64_MAX; // Return -1 to indicate failure
    }
	...
	while (!params->worker_done) {
		...
		switch (req.request.img_op) {
			...
			case IMG_RETRIEVE:{
				struct image * img = params->the_queue->image_store[req.request.img_id];
				if (img != NULL) {
					int bytes_sent = sendImage(img, params->conn_socket);
					if (bytes_sent != 0) {
						resp.img_id = req.request.img_id;
						resp.ack = RESP_REJECTED;
					} else {
		 ...
		 * report. */
		sync_printf("T%d R%ld:%lf,%u,%u,%ld,%ld,%lf,%lf,%lf\n", 
			params->worker_id,
			req.request.req_id,
		...
		if (in_bytes > 0) {
			...
			if (req->request.img_op == IMG_REGISTER) {
				...
				if (new_image != NULL) {
					...
					sync_printf("R%ld:%lf,IMG_REGISTER,%u,%ld,%ld,%lf,%lf,%lf\n", 
						req->request.req_id,
						TSPEC_TO_DOUBLE(req->request.req_timestamp),
						req->request.overwrite,
						(uint64_t)0,
