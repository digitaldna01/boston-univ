...
struct request {
	...
	union {
		...
		struct {
			uint8_t  img_op;
			uint8_t  overwrite; // 8-bit value set to 0 or 1 to indicate whether the operation requested by the client should overwrite or not th original image
			uint64_t img_id; // 64 bit image identifier 
		};
	};
};
